...
function updateShareCodes(url = '') {
  return new Promise(resolve => {
    $.get({ url, headers: { "User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./utils/USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1") }, timeout: 200000 }, async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    $.newShareCodes = ['e5f0d0de-4032-4da7-bdeb-a992256a55be@e89ee3e4-c5ea-45e5-b5bc-21058f41620e@07b782df-1b5c-44b1-9b44-0200770ef3ae@63c1e5e2-04b1-48c3-bfea-cf286b61b36b@499b337a-7bf1-441a-9cf6-18200a48ca86@2a9f4878-b3a5-40f2-ad52-a60c665e855c'];
